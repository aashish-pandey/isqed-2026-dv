//=============================================================================
// gpio_scoreboard.sv
// Checks DUT responses against the GPIO reference model predictions.
// Subscribes to TL-UL monitor write and read ports.
// On every observed read, compares DUT rdata against predicted value.
// On every write to DATA_OUT/DIR, verifies gpio_o/gpio_oe_o output pins.
//=============================================================================
`ifndef GPIO_SCOREBOARD_SV
`define GPIO_SCOREBOARD_SV

class gpio_scoreboard extends uvm_scoreboard;
  `uvm_component_utils(gpio_scoreboard)

  // ── Analysis FIFOs — connected from tl_ul_monitor ─────────────────────────
  uvm_analysis_imp_write #(tl_ul_seq_item, gpio_scoreboard) aimp_write;
  uvm_analysis_imp_read  #(tl_ul_seq_item, gpio_scoreboard) aimp_read;

  // Virtual interface for sampling DUT output pins
  virtual interface tl_ul_if tl_vif;
  virtual interface gpio_if   gpio_vif;

  // ── Internal shadow registers (mirror RTL state) ──────────────────────────
  logic [31:0] shadow_data_out    = 32'h0;
  logic [31:0] shadow_dir         = 32'h0;
  logic [31:0] shadow_intr_state  = 32'h0;
  logic [31:0] shadow_intr_enable = 32'h0;
  logic [31:0] shadow_intr_ctrl_rising  = 32'h0;
  logic [31:0] shadow_intr_ctrl_falling = 32'h0;
  logic [31:0] shadow_intr_ctrl_lvlhigh = 32'h0;
  logic [31:0] shadow_intr_ctrl_lvllow  = 32'h0;

  // ── Counters ──────────────────────────────────────────────────────────────
  int unsigned checks_passed = 0;
  int unsigned checks_failed = 0;

  // ── Register address constants ────────────────────────────────────────────
  localparam logic [31:0] ADDR_DATA_IN              = 32'h00;
  localparam logic [31:0] ADDR_DATA_OUT             = 32'h04;
  localparam logic [31:0] ADDR_DIR                  = 32'h08;
  localparam logic [31:0] ADDR_INTR_STATE           = 32'h0C;
  localparam logic [31:0] ADDR_INTR_ENABLE          = 32'h10;
  localparam logic [31:0] ADDR_INTR_TEST            = 32'h14;
  localparam logic [31:0] ADDR_INTR_CTRL_EN_RISING  = 32'h18;
  localparam logic [31:0] ADDR_INTR_CTRL_EN_FALLING = 32'h1C;
  localparam logic [31:0] ADDR_INTR_CTRL_EN_LVLHIGH = 32'h20;
  localparam logic [31:0] ADDR_INTR_CTRL_EN_LVLLOW  = 32'h24;
  localparam logic [31:0] ADDR_MASKED_OUT_LOWER     = 32'h28;
  localparam logic [31:0] ADDR_MASKED_OUT_UPPER     = 32'h2C;

  function new(string name, uvm_component parent);
    super.new(name, parent);
  endfunction

  function void build_phase(uvm_phase phase);
    super.build_phase(phase);
    aimp_write = new("aimp_write", this);
    aimp_read  = new("aimp_read",  this);
    if (!uvm_config_db #(virtual tl_ul_if)::get(this, "", "tl_vif", tl_vif))
      `uvm_fatal("NO_VIF", "gpio_scoreboard: tl_vif not found")
    if (!uvm_config_db #(virtual gpio_if)::get(this, "", "gpio_vif", gpio_vif))
      `uvm_fatal("NO_VIF", "gpio_scoreboard: gpio_vif not found")
  endfunction

  // ── Write transaction handler ─────────────────────────────────────────────
  function void write_write(tl_ul_seq_item txn);
    logic [31:0] addr  = {txn.addr[31:2], 2'b00};
    logic [31:0] wdata = txn.data;

    // Update shadow state
    case (addr)
      ADDR_DATA_OUT: shadow_data_out = wdata;

      ADDR_DIR:      shadow_dir = wdata;

      ADDR_INTR_STATE: begin
        // W1C: clear bits written as 1
        shadow_intr_state = shadow_intr_state & ~wdata;
      end

      ADDR_INTR_ENABLE:          shadow_intr_enable = wdata;
      ADDR_INTR_CTRL_EN_RISING:  shadow_intr_ctrl_rising  = wdata;
      ADDR_INTR_CTRL_EN_FALLING: shadow_intr_ctrl_falling = wdata;
      ADDR_INTR_CTRL_EN_LVLHIGH: shadow_intr_ctrl_lvlhigh = wdata;
      ADDR_INTR_CTRL_EN_LVLLOW:  shadow_intr_ctrl_lvllow  = wdata;

      ADDR_MASKED_OUT_LOWER: begin
        for (int i = 0; i < 16; i++) begin
          if (wdata[i+16]) shadow_data_out[i] = wdata[i];
        end
      end

      ADDR_MASKED_OUT_UPPER: begin
        for (int i = 0; i < 16; i++) begin
          if (wdata[i+16]) shadow_data_out[i+16] = wdata[i];
        end
      end

      default: ; // DATA_IN writes silently ignored
    endcase

    // Verify gpio_o and gpio_oe_o match shadow after output-affecting writes
    if (addr inside {ADDR_DATA_OUT, ADDR_DIR, ADDR_MASKED_OUT_LOWER, ADDR_MASKED_OUT_UPPER}) begin
      check_output_pins();
    end

    // Check no unexpected bus error
    if (txn.error)
      `uvm_error("SB_ERR", $sformatf("Unexpected bus error on WRITE addr=0x%08h", addr))
  endfunction

  // ── Read transaction handler ──────────────────────────────────────────────
  function void write_read(tl_ul_seq_item txn);
    logic [31:0] addr     = {txn.addr[31:2], 2'b00};
    logic [31:0] dut_data = txn.rdata;
    logic [31:0] exp_data;
    string context_str;

    context_str = $sformatf("addr=0x%08h", addr);

    case (addr)
      ADDR_DATA_OUT:             exp_data = shadow_data_out;
      ADDR_DIR:                  exp_data = shadow_dir;
      ADDR_INTR_STATE:           exp_data = shadow_intr_state;
      ADDR_INTR_ENABLE:          exp_data = shadow_intr_enable;
      ADDR_INTR_CTRL_EN_RISING:  exp_data = shadow_intr_ctrl_rising;
      ADDR_INTR_CTRL_EN_FALLING: exp_data = shadow_intr_ctrl_falling;
      ADDR_INTR_CTRL_EN_LVLHIGH: exp_data = shadow_intr_ctrl_lvlhigh;
      ADDR_INTR_CTRL_EN_LVLLOW:  exp_data = shadow_intr_ctrl_lvllow;
      ADDR_MASKED_OUT_LOWER:     exp_data = {16'h0, shadow_data_out[15:0]};
      ADDR_MASKED_OUT_UPPER:     exp_data = {16'h0, shadow_data_out[31:16]};
      ADDR_DATA_IN: begin
        // DATA_IN depends on gpio_i synchronizer — don't predict exact value,
        // just check no error
        if (txn.error)
          `uvm_error("SB_ERR", "Unexpected error on DATA_IN read")
        return;
      end
      default: begin
        exp_data = 32'h0;  // unknown address returns 0
      end
    endcase

    if (dut_data !== exp_data) begin
      checks_failed++;
      `uvm_error("SB_MISMATCH",
        $sformatf("READ %s: DUT=0x%08h EXPECTED=0x%08h", context_str, dut_data, exp_data))
    end else begin
      checks_passed++;
      `uvm_info("SB_OK",
        $sformatf("READ %s: 0x%08h ✓", context_str, dut_data), UVM_HIGH)
    end
  endfunction

  // ── Output pin checker ────────────────────────────────────────────────────
  function void check_output_pins();
    // gpio_o should equal shadow_data_out
    if (gpio_vif.gpio_o !== shadow_data_out) begin
      checks_failed++;
      `uvm_error("SB_GPIO_O",
        $sformatf("gpio_o mismatch: DUT=0x%08h EXP=0x%08h",
                  gpio_vif.gpio_o, shadow_data_out))
    end else begin
      checks_passed++;
    end

    // gpio_oe_o should equal shadow_dir
    if (gpio_vif.gpio_oe_o !== shadow_dir) begin
      checks_failed++;
      `uvm_error("SB_GPIO_OE",
        $sformatf("gpio_oe_o mismatch: DUT=0x%08h EXP=0x%08h",
                  gpio_vif.gpio_oe_o, shadow_dir))
    end else begin
      checks_passed++;
    end
  endfunction

  function void report_phase(uvm_phase phase);
    `uvm_info("SB_REPORT",
      $sformatf("GPIO Scoreboard: %0d passed, %0d failed",
                checks_passed, checks_failed), UVM_NONE)
    if (checks_failed > 0)
      `uvm_error("SB_FAIL", "Scoreboard detected mismatches — see above")
  endfunction

endclass

`endif // GPIO_SCOREBOARD_SV
